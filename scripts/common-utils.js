...
// product fallback image path

export const productFallbackImage = '/content/dam/danaher/products/fallbackImage.jpeg';
/*
...
export function showPreLoader() {
  ...
  if (mainPreLoader) {
    const loaderPaymentText = mainPreLoader?.querySelector('h3');
    if (loaderPaymentText) {
      loaderPaymentText.textContent = '';
    }
  }
}
...
export function preLoader() {
  const preloaderContainer = div(
    {
      class:
        'fixed flex-col top-0 left-0 flex items-center justify-center w-screen h-screen z-[100] backdrop-blur-sm',
      id: 'preLoader',
    },
  );
  ...
  preloaderContainer?.insertAdjacentElement('beforeend', h3({ class: 'text-xl text-danaherpurple-500' }, ''));
...
export function createModal(content, hasCancelButton, hasCloseButton, dataType = '', dataAction = '') {
  ...
  const modalContainer = div({
    class: 'relative max-w-2xl max-h-[97vh] w-full items-center bg-white p-3 md:p-6',
  ...
  if (hasCancelButton === true) {
    ...
    if (content && modalBody) {
      ...
      if (getModalButtonWrapper) {
        ...
        getModalButtonWrapper.insertAdjacentElement('afterbegin', cancelButton);
      }
    }
  ...
  if (hasCloseButton) {
    const modalCloseButton = p(
      {
        class: 'close-button absolute right-10 top-8',
        name: 'close',
      },
      span({
...
/**
 * Apply responsive paddingTop to a block.
 * @param {HTMLElement} block - The DOM element to apply padding on.
 * @param {string} mobilePadding - PaddingTop value for mobile (<= 767px).
 * @param {string} desktopPadding - PaddingTop value for desktop (> 767px). Defaults to '0px'.
 */
export function applyResponsivePadding(block, mobilePadding = '8px', desktopPadding = '0px') {
  function updatePadding() {
    block.style.paddingTop = window.matchMedia('(max-width: 767px)').matches
      ? mobilePadding
      : desktopPadding;
  }
 ...
 */
export async function getProductInfo(id, needInterShop = true, hasBrand = true) {
  const api = true;
  ...
  if (api) {
    try {
      ...
      if (coveoResponse?.status === 'success') {
        ...
        if (needInterShop) {
          let intershopProductId = '';
          if (hasBrand) {
            intershopProductId = id.slice(0, id.lastIndexOf('-'));
          } else {
            intershopProductId = id;
          }
...
export const buildButton = (buttonLabel, id, classes) => div(
  { class: 'space-y-2 button-wrapper mt-6 flex items-center gap-4' },
  button(
    {
      type: 'button',
      class: classes,
...
export const buildInputElement = (
  ...
  return div(
    {
      class: `space-y-2 field-wrapper relative  ${hiddenField} ${classes} w-full`,
    },
    label(
      {
        for: fieldLable,
...
export const countriesArray = () => {
  const countriesList = [
    ...
    { 'name': 'Palestine State', 'isoCode': 'PS' },
...
// get country name by isoCode
export function getCountryNameByIsoCode(isoCode) {
  const country = countriesArray().find((c) => c.isoCode === isoCode);
