...
/*
check for url to load stripe
*/
export function canLoadStripe() {
  const allowedPaths = [
    window.EbuyConfig?.paymentPageUrl,
    window.EbuyConfig?.paymentMethodsPageUrl,
  ];
...
export async function loadStripe() {
  ...
  if (!stripeInstance) {
    ...
    stripeInstance = Stripe(window.EbuyConfig?.stripeKey);
  }
  return stripeInstance;
}
// get payment intent
export async function getPaymentIntent(update = '') {
  const authenticationToken = await getAuthenticationToken();
  if (authenticationToken?.status === 'error') {
    return { status: 'error', data: 'Unauthorized access.' };
  }
  let cachedIntent = true;
  if (update === 'updateIntents') {
    cachedIntent = false;
  }
  if (cachedIntent?.status === 'success') {
    const getCachedIntent = JSON.parse(sessionStorage.getItem('savedIntents'));
    if (getCachedIntent) {
      return getCachedIntent;
    }
  }
  // get payment intent
  ...
  sessionStorage.setItem('savedIntents', JSON.stringify(response));
  return response;
}
...
export async function updateSavedCards() {
  return getPaymentIntent('updateIntents');
}
