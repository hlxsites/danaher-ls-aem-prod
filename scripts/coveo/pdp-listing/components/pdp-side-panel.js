...
function createFacet(facet, isFirst = false) {
  ...
  const title = createEl('div', {
    className: 'text-black text-base leading-[22px] font-bold',
    text: facet.title,
  });
  ...
  nodes.forEach((node) => {
    ...
    const label = createEl('div', {
      className: `text-sm text-black py-1 flex items-center gap-2 ${
  ...
  function renderCheckboxItems(filterText = '') {
    ...
    visibleItems.forEach((item) => {
      ...
      const label = createEl('label', {
        className: 'flex items-center gap-2 text-[14px] leading-[20px] font-normal text-black',
      });
...
const expandCollapseAll = createEl('button', {
  className:
    'text-[#7523FF] text-right text-[16px] leading-[22px] font-bold hover:underline flex items-center gap-2 ml-auto',
...
export function createFiltersPanel() {
  ...
  const title = createEl('h2', {
    className:
      'font-semibold flex items-center gap-2 text-black text-[32px] leading-[40px] font-normal',
  ...
  const viewBtn = createEl('button', {
    className:
      'w-full py-2 bg-danaherpurple-500 rounded-full hover:bg-purple-700 text-white text-base leading-[22px] font-normal',
    events: {
      click() {
        panel.style.transform = 'translateX(100%)';
        modalOverlay.classList.add('hidden');
