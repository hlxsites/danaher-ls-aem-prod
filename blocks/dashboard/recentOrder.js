...
const recentOrders = async () => {
  ...
  const recentDetailsContainer = div({
    class: 'self-stretch inline-flex  md:flex-row flex-col justify-start items-start gap-1',
  });
  ...
  const topThreeOrdersDiv = (order, po, status, creationDate, orderTotal, orderId) => {
  ...
  const topQuotesDiv = (quoteNo, creationDate, quoteId) => {
    const date = new Date(creationDate);
    const month = String(date.getMonth() + 1).padStart(2, '0'); // Months are 0-based
    const day = String(date.getDate()).padStart(2, '0');
    const year = date.getFullYear();
    ...
    const formattedDate = `${month}/${day}/${year}`;
    const topRequestedQuoteDiv = a(
      {
        class:
          'self-stretch py-3 bg-white border-b border-gray-200 inline-flex justify-start items-start',
        href: `${window.EbuyConfig?.requestedQuoteDetailsPageUrl}?quoteId=${quoteId}`,
      },
      div(
        {
          class: 'flex-1 inline-flex flex-col justify-start items-start gap-1',
  ...
  if (requestedQuotesResponse?.length === 0) {
    ...
    topRequestedQuote?.forEach((element) => {
      ...
      const quoteId = element.title;
      const quoteDiv = topQuotesDiv(quote.quoteNo, quote.creationDate, quoteId);
