...
*/
      if (getNotes?.value === undefined && getShippingNotesField?.value?.trim() === '') {
        return false;
      }
      showPreLoader();
      /*
...
const shippingMethodsModule = async () => {
  if (!window.location.pathname === window.EbuyConfig?.shippingPageUrl) return false;
  ...
  try {
    ...
    */
    ...
    const moduleHeader = div(
      ...
      h2({
        class: 'text-black text-left text-3xl font-dhlsMedium leading-12 p-0 m-0  pb-6',
        ...
        textarea(
          {
            ...
            cols: '50',
            class:
              'input-focus outline-none text-base w-full block px-2 py-4  border border-solid border-gray-400',
            'aria-label': 'notes',
            label: 'Notes',
