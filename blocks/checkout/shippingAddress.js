...
function initGmapsAutocomplete(addressType, addressInput = '') {
  ...
  autocomplete.addListener('place_changed', async () => {
    ...
    const getFormId = document.querySelector(`#${addressType}-address-form`);
    if (getFormId) {
 ...
 */
const renderAddressList = (addressItems, addressListArray, type, showEmptyActions = '', searchTerm = '') => {
  if (typeof addressListArray !== 'undefined' && addressListArray.length > 0) {
    ...
    const checkUseAddresses = JSON.parse(localStorage.getItem('useAddress'));
        ...
        */
        ...
        if ((item?.preferredShippingAddress === 'true' && type === 'shipping') || (type === 'billing' && item?.preferredBillingAddress === 'true')) {
          makeDefaultButton = div(
            {
              class: `flex justify-between items-center gap-3 is-default-${type}-address cursor-pointer`,
            },
            input(
              {
                type: 'checkbox',
          ...
          makeDefaultButton = div(
            {
              class: `relative flex gap-3 text-right is-default-${type}-address cursor-pointer`,
            },
            input(
              {
                type: 'checkbox',
        ...
        const addressListItem = div(
          {
            ...
            class: `:hover:bg-gray-100 flex flex-col justify-between p-6 gap-6 border ${selectedAddress ? 'selected-address' : 'border-danahergray-300'}  ${type}-address-list-item ${defaultBgClass}`,
          },
          div(
            {
              class: `flex ${type}-address-list-item-content w-full`,
          ...
          div(
            {
              class: `${type}-address-list-item-actions flex items-start md:items-center gap-6 flex-col md:flex-row justify-between`,
            ...
            button(
              {
                id: item.id,
                class: `${type}-address-use-button ${selectedAddress ? 'btn-primary-purple' : 'border-danaherpurple-500  bg-white btn-outline-primary'}  !mt-0 text-xl  border-solid btn btn-lg font-medium rounded-full px-6`,
              },
...
:
    ...
    const emptyAddressListWrapper = div(
      {
        class: 'flex flex-col justify-between items-center w-full !gap-2',
      },
      div(
        {
          class: ' flex justify-start w-[60px] h-[60px] items-center gap-3',
        },
        span({
          id: 'address-location-icon',
          class: 'icon icon-Location-marker w-[60px] h-[60px] [&_svg>use]:stroke-danaherpurple-500 [&_svg>use]:hover:stroke-danaherpurple-800',
        }),
      ),
      h2(
        {
          class: 'text-3xl text-black text-demobold m-0',
        },
        'We’re sorry.',
      ),
      h3(
        {
          class: 'text-black text-center flex items-center m-0 justify-center',
        },
        `We could not find a Address match for "${searchTerm}"`,
      ),
      p(
        {
      ...
      div(
        {
          class: 'flex w-full justify-center gap-4 items-center mt-6 !hidden',
        },
    ...
    decorateIcons(emptyAddressListWrapper);
...
export const addressListModal = async (type, cancelButton = true) => {
  ...
  const addressListHeader = div(
    ...
    p(
      {
        class: 'font-dhlsMedium text-3xl text-black',
      },
      'My Addresses',
    ),
    div(
      {
        class: 'flex justify-between items-center my-4 py-6 flex-col md:flex-row gap-6',
  ...
  const addressItems = div({
    class:
      'max-h-[65vh] overflow-auto flex flex-col gap-6 pt-0 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-gray-100 [&::-webkit-scrollbar-thumb]:bg-gray-300 dark:[&::-webkit-scrollbar-track]:bg-neutral-700 dark:[&::-webkit-scrollbar-thumb]:bg-neutral-500',
  ...
  */
  ...
  if (addressListSearchInput) {
    addressListSearchInput.addEventListener('input', (e) => {
      ...
      renderAddressList(addressItems, searchedAddress, type, showEmptyActions, searchTerm);
    });
  }
...
document.addEventListener('click', async (e) => {
  ...
  if (e.target.matches('#close-utility-modal')) {
...
async function generateDefaultAddress(
  ...
  if (invoiceToAddress?.status === 'success' && invoiceToAddress?.data?.length > 0) {
    const address = invoiceToAddress?.data?.filter(
...
export const shippingAddressModule = async () => {
  try {
    if (!window.location.pathname === window.EbuyConfig?.addressPageUrl) return false;
    if (authenticationToken?.status === 'error') {
      throw new Error('Unauthorized Access');
    }
    // load google maps script
    await loadGmapsScript(window.EbuyConfig?.googleMapsApi);
    const validateData = {
   ...
   */
    const getDefaultAddressesResponse = await getAddresses('shipping');
   ...
   */
    ...
    */
    ...
    const basketInvoiceToAddress = getCurrentBasketDetails?.data?.data?.invoiceToAddress;
    const basketShipToAddress = getCurrentBasketDetails?.data?.data?.commonShipToAddress;
    if (!basketShipToAddress && basketInvoiceToAddress && shippingAsBillingAddress) {
      shippingAsBillingAddress?.classList?.add('pointer-events-none');
      shippingAsBillingAddress?.classList?.add('opacity-50');
      if (shippingAsBillingAddressInput) {
        shippingAsBillingAddressInput.checked = false;
        shippingAsBillingAddressInput.value = false;
      }
    }
 ...
 */
  ...
  */
      ...
      const billingAddressFormWrapper = document.querySelector(
  ...
  */
      ...
      const getUseAddressesResponseI = await getUseAddresses();
      const useShipToAddressI = getUseAddressesResponseI?.data?.commonShipToAddress;
      const useInvoiceToAddressI = getUseAddressesResponseI?.data?.invoiceToAddress;

      if (targetCheckbox.value === 'true' && targetFrom === 'label' && !basketInvoiceToAddress && !basketShipToAddress) {
        // showDefaultBillingAddress?.classList.add('hidden');
        billingAddressForm?.classList?.remove('hidden');
        billingAddressFormWrapper?.classList?.remove('hidden');
        ...
        if (!basketInvoiceToAddress && billingAddressForm) {
          ...
          billingAddressFormWrapper?.classList?.add('hidden');
        }

        if (
          useInvoiceToAddressI?.urn?.split(':')?.pop()
          !== useShipToAddressI?.urn?.split(':')?.pop()
        ) {
          showPreLoader();
          /*
  ...
  */
          ...
          const setAddressDetails = {
            firstName:
              useShipToAddressI?.firstName
              ?? '',
            lastName:
              useShipToAddressI?.lastName
              ?? '',
            companyName2:
              useShipToAddressI
                ?.companyName2 ?? '',
...
*/
        ...
        if (shippingAsBillingAddressInput) {
          setTimeout(() => {
            const getShipAsBillBox = document.querySelector('#shippingAsBillingCheckboxWrapper');
            if (getShipAsBillBox && (invoiceTo === shipTo)) {
              getShipAsBillBox?.classList.add('pointer-events-none');
