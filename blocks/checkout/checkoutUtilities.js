...
export const progressModule = () => {
  ...
  */
  const progressBar = div({
    ...
    class:
      'checkout-progress-bar flex items-center justify-between mb-16 relative w-full md:max-w-[65%] mx-auto',
  });

  const checkoutHeading = h2(
    {
      class: 'text-4xl md:w-96 font-semibold text-black',
    },
    'Checkout',
  );
 ...
 */
  const progressBarWrapper = div(
    {
      class: 'flex  flex-col md:flex-row w-full gap-6 p-6 md:p-3',
    },
  );
  progressBar.append(line, segment1, segment2, address, shipping, payment);
  progressBarWrapper.append(checkoutHeading, progressBar);
...
export const initializeModules = async () => {
  ...
  if (checkPath === window.EbuyConfig?.addressPageUrl) updateCheckoutUI(steps, 'addresses');
  else if (checkPath === window.EbuyConfig?.shippingPageUrl) updateCheckoutUI(steps, 'shipping');
  else if (checkPath === window.EbuyConfig?.paymentPageUrl) updateCheckoutUI(steps, 'payment');
  else if (checkPath === window.EbuyConfig?.paymentMethodsPageUrl) updateCheckoutUI(steps, 'payment');

  if (checkPath === window.EbuyConfig?.addressPageUrl) {
  ...
  if (checkPath === window.EbuyConfig?.shippingPageUrl) {
  ...
  if (checkPath === window.EbuyConfig?.paymentPageUrl) {
