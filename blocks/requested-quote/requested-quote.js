...
export default async function decorate(block) {
  ...
  const dynamicTableContent = async (requestedQuotesResp) => {
    const tableRow = (quoteId, quoteNo, creationDate, brand) => {
      ...
      return a(
        {
          class: 'inline-flex justify-start border-b border-gray-200 gap-1 md:w-full w-[980px]',
    ...
    const tableRows = await Promise.all(
      requestedQuotesResp?.map(async (element) => {
        const quote = {};
