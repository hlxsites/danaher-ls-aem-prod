...
async function getItems() {
  ...
  const itemPaths = pathParts.length > 2
    ? pathParts.slice(3).map((_, i) => pathParts.slice(0, i + 4).join('/'))
    : [];
  ...
  return itemPaths.map((itemPath) => {
    ...
    let title = article && article.navTitle !== ''
      ? article.navTitle
      : itemPath.split('/').pop();
...
export default async function decorate(block) {
  if (!block.querySelector('div > ul')) {
    ...
    block.innerHTML = '';
    block.append(div({}, div({}, ul({}, ...listItems))));
  }
  ...
  */
    ...
    if ((includeProdEdsPaths.some((prodPath) => window.location.pathname.includes(prodPath)) || (includeStageEdsPaths.some((stagePath) => window.location.pathname.includes(stagePath)) && window.DanaherConfig.host.includes('stage.lifesciences'))) && index > 1) {
      ...
      if (formattedElement.toLowerCase() === 'brands') {
        elementAnchorContent?.setAttribute('href', '/us/en/products.html');
      ...
      if (formattedElement.toLowerCase() === 'news-eds') {
        if (elementAnchorContent) {
          formattedElement = 'News';
        }
      }
      if (formattedElement.toLowerCase() === 'blog-eds') {
        if (elementAnchorContent) {
          formattedElement = 'Blog';
        }
      }
      if (window.location.pathname.includes('brands')) {
        if (index === arr.length - 1) {
          formattedElement = document.title && document.title.trim()
            ? document.title.split('|')[0].trim()
            : elementContent.split('|')[0].trim();
        }
      }
