...
export default async function decorate(block) {
  ...
  // Capture the authored text before clearing the block
  const authoredText = block.textContent.trim() || 'You may also be interested in';
  ...
  const spanEl = articles.length > 0 ? h2({ class: 'text-lg font-semibold' }, authoredText) : '';
  const spanE2 = articles.length > 0 ? a({ class: 'my-auto text-base text-danaherpurple-500 font-semibold', href: goBack() }, 'See all →') : '';
