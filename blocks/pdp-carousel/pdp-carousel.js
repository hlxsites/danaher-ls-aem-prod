...
export default async function decorate(block) {
  ...
  const sku = response?.raw?.sku;

  if (!sku) return;

  const storageKey = `pdp-carousel-products-${sku}`;
  const cachedProducts = JSON.parse(localStorage.getItem(storageKey));

  Object.keys(localStorage).forEach((key) => {
    if (key.startsWith('pdp-carousel-products-') && key !== storageKey) {
