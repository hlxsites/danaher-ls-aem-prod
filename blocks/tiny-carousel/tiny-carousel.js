...
export default async function decorate(block) {
  ...
  const titleText = block.querySelector('[data-aue-prop="titleText"]')?.textContent.trim()
    || 'Continue Browsing';
  ...
  const scrollToIndex = (sIndex) => {
    const card = scrollContainer.children[0];
    if (!card) return;
    const cardWidth = card.offsetWidth + 16;
    scrollContainer.style.transform = `translateX(-${cardWidth * sIndex}px)`;
    currentIndex = sIndex;
    updateArrows();
  };
