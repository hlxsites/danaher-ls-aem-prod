...
export default async function decorate(block) {
  ...
  function updatePadding() {
    block.style.paddingTop = window.matchMedia('(max-width: 767px)').matches ? '32px' : '0px';
  }
  ...
  resourceResultList.subscribe(() => {
    ...
    const renderPagination = () => {
      ...
      prevButton.addEventListener('click', () => {
        ...
        const target = document.getElementById('resources-tab');
        if (target) {
          const yOffset = 100; // Adjust this value for more/less space at top
    ...
    const head = div({ class: 'block-pdp-resources text-2xl leading-8 font-medium', style: 'font-weight: 400 !important;' }, 'Resources');
