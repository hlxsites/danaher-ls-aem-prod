      ...
      if (response.status === 'success') {
        ...
        array.some((obj) => {
          if (Object.prototype.hasOwnProperty.call(obj, manufacturer)) {
            const exists = obj[manufacturer].some(
              (p) => p.lineItemId === product.lineItemId,
            );
...
export const updateCartQuantity = (newQuantity) => {
  ...
  const recommendedProductDiv = document.querySelector('.recommendedProduct');
  if (cartItems) {
    ...
    if (newQuantity === 0) {
      ...
      recommendedProductDiv.querySelector('#othersBought').classList.add('hidden');
      recommendedProductDiv.querySelector('#recentlyVisited').classList.remove('hidden');
    } else {
      ...
      recommendedProductDiv.querySelector('#othersBought').classList.remove('hidden');
      recommendedProductDiv.querySelector('#recentlyVisited').classList.add('hidden');
    }
  }
