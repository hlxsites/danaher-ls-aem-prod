...
export default async function decorate(block) {
  ...
  const dynamicQuoteDetail = (approvedQuotesDetailsResponse) => {
    ...
    const quoteDetail = div(
      {
        class:
          'w-[930px] h-96 p-6 bg-gray-50 inline-flex flex-col justify-start items-start gap-6',
    ...
    const dynamicQuoteDetailContent = (productSKU, productName, unitOfMeasure, quantity, singlePrice) => {
      const formattedAmount = parseFloat(singlePrice).toLocaleString('en-US', {
    ...
    approvedQuotesDetailsResponse.items?.map(async (element) => {
      const productDetails = await productData(element.productSKU);
      const UOM = productDetails.data.packingUnit === '' ? 'NA' : productDetails.data.packingUnit;

      // eslint-disable-next-line max-len
      const quoteDetailRows = dynamicQuoteDetailContent(element.productSKU, productDetails.data.productName, UOM, element.originQuantity.value, element.originSinglePrice.value);
  ...
  const dynamicTableContent = async (approvedQuotesResp) => {
    const tableRow = (
      ...
      const approvedQuoteRow = div(
        {
          class: 'flex flex-col md:w-full w-[980px] ',
